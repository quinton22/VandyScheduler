import { Course, CourseSection } from './course/course';
...
export class Schedule {
  ...
  addCourse(c: Course) {
    this.classesArr.push(c);
    this.sortClasses();
    this.checkOverlapOfAllClasses();
  }

  getAllPossibleSchedules() {
    return this.scheduleArr.slice();
  }
  ...
  static checkOverlap(currentClass: CourseSection, schedule: CourseSection[]) {
    return schedule.some((section) => section.overlapsWith(currentClass));
  ...
  checkOverlapOfAllClasses() {
    ...
    for (const section of allSections) {
      this.overlappedClasses.set(
        ...
        section.getNumOverlaps(
          allSections.filter(
            (s) => s.course?.classAbbr !== section.course?.classAbbr
          )
        )
      );
    }
