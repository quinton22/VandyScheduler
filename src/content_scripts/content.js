 ...
 */
export function getFromStorageAndCreateModal(prefModal) {
  ...
  $(checkBoxDiv)
    ...
    .each((_, el) => {
      ...
      el.onchange = () => {
        preferences.noPrefMet = !JSON.parse(el.value);
      };
    });
    ...
    try {
      ...
      if (storedClasses) {
        ...
        currentClasses.forEach((v, k) =>
          storedClasses.has(k) ? intersection.push(k) : null
        );
 ...
 */
export function sortBasedOnPreferences(arr) {
  ...
  if (preferences.noPrefMet) {
    ...
    arr = arr.filter((sched) => {
      let flag = true;
      breakArr.forEach((c) => {
        if (Schedule.checkOverlap(c, sched)) flag = false;
      });
      return flag;
    });
  } else {
 ...
 */
export function createViewableContent(arr, tbaClasses, overlappedClasses) {
  var scheduleDiv;
  if (arr.length > 0) {
      ...
      pickSchedBtn.addEventListener('click', () => {
        ...
        curSched.forEach((c) => {
          ...
          let key = Array.from(parent2).findIndex((el) =>
            el.children[0].innerText.includes(c.classAbbr)
          );
 ...
 */
export function getClass(classAbbr, section) {
 ...
 */
export function placeClass(classDiv, scheduleDiv, day, time) {
